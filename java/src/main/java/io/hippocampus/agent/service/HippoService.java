...
public class HippoService {
    ...
    public List<Hippo> addHippo(final Hippo hippo) {
        if (hippo != null) {
            ...
            LOGGER.log(INFO, "Adding a new hippo with hippo being {0} caracter and having {1} tags",
    ...
    public List<Hippo> addHippo(final String hippo, final String tags) {
        if (isBaseInputValid(hippo)) {
             ...
             * at the view screen.
             */
            LOGGER.log(WARNING, "Attempted to add a hippo but the input was invalid, the input being: {0}, {1}, {2}",
                    (hippo != null ? hippo.length() : "null"), (tags != null ? tags.length() : "null"));
        }
